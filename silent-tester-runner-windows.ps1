...
param (
  ...
  [Alias ("Seconds")]
  [int] $ScenarioDuration = 85500, # Default is 85500 seconds, or 23.75 hours.
  [Alias ("SCCM","Intune")]
...
$edgeHeadlessPolicy = Get-ItemProperty -Path $edgePoliciesPath -Name HeadlessModeEnabled -ErrorAction SilentlyContinue
    ...
    elseif ($isChrome) {
        ...
        # But we can check for policies that might interfere with automation
        $chromeAutomationPolicy = Get-ItemProperty -Path $chromePoliciesPath -Name DeveloperToolsAvailability -ErrorAction SilentlyContinue
        if ($IsHeadless -and $null -ne $chromeAutomationPolicy -and $chromeAutomationPolicy.DeveloperToolsAvailability -eq 2) {
            Out-Log "WARNING: Chrome policy 'DeveloperToolsAvailability' is set to 2 (Disabled). This may interfere with headless runner functionality." -ForegroundColor Yellow
        }
    }
    else {
...
while ((`$now = Get-Date) -lt `$endTime) {
  ...
  try {
    ...
    Log-WatchdogMessage \"Error with Write-Progress: `$_\"
  }
  
  Start-Sleep -Seconds 1

  try {
    `$processCheck = Get-Process -Id $chromePid -ErrorAction SilentlyContinue
    if (-not `$processCheck) {
    ...
    Log-WatchdogMessage \"Error checking process: `$_\"
  }
}
...
try {
  ...
  try {
    ...
    Log-WatchdogMessage \"Failed to stop chromium process\"
    Log-WatchdogMessage \"Error: `$_\"
  }
} catch {
  if (!`$endedPrematurely) {
